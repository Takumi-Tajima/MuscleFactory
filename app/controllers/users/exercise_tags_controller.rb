class Users::ExerciseTagsController < Users::ApplicationController
  ...
  def create
    ...
    if @exercise_tag.save
      redirect_to users_exercise_tag_path(@exercise_tag), notice: 'タグを作成しました'
    else
  ...
  def update
    if @exercise_tag.update(exercise_tag_params)
      redirect_to users_exercise_tag_path(@exercise_tag), notice: 'タグを更新しました'
    else
  ...
  def destroy
    ...
    redirect_to users_exercise_tags_path, notice: 'タグを削除しました', status: :see_other
  end
