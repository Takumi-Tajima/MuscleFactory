class Users::MuscleGroupsController < Users::ApplicationController
  ...
  def create
    ...
    if @muscle_group.save
      redirect_to users_muscle_group_path(@muscle_group), notice: '部位を作成しました'
    else
  ...
  def update
    if @muscle_group.update(muscle_group_params)
      redirect_to users_muscle_group_path(@muscle_group), notice: '部位を更新しました'
    else
  ...
  def destroy
    ...
    redirect_to users_muscle_groups_path, notice: '部位を削除しました', status: :see_other
  end
