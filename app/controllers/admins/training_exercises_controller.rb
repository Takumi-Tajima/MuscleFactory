class Admins::TrainingExercisesController < Admins::ApplicationController
  ...
  def create
    ...
    if @training_exercise.save
      redirect_to admins_training_exercise_path(@training_exercise), notice: '種目を作成しました'
    else
  ...
  def update
    if @training_exercise.update(training_exercise_params)
      redirect_to admins_training_exercise_path(@training_exercise), notice: '種目を更新しました'
    else
  ...
  def destroy
    ...
    redirect_to admins_training_exercises_path, notice: '種目を削除しました', status: :see_other
  end
